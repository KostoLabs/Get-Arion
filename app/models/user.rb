class User < ApplicationRecord
  attr_accessor :siren_temp, :company_name_temp, :company_address_temp,
              :company_postal_code_temp, :company_city_temp,
              :company_naf_temp, :company_creation_date_temp

  after_create :create_corporate_from_user_data
  has_secure_password
  ...
  # validates :company_name, :company_address, :company_naf, :company_creation_date,
  #         presence: true, if: ->(user) { user.siren.present? }
  ...
  private
    ...
    def totp
      ROTP::TOTP.new(otp_secret, issuer: "Arion")
    end
    ...
    def create_corporate_from_user_data
      return if siren_temp.blank? && company_name_temp.blank?

      family.create_corporate!(
        user: self,
        siren: siren_temp,
        name: company_name_temp,
        address: company_address_temp,
        naf: company_naf_temp,
        creation_date: company_creation_date_temp
      ).tap do |corporate|
        corporate.update!(
          address: company_address_temp,
          naf: company_naf_temp,
