class Provider::Github
  ...
  def initialize(config = {})
    @name = config[:name] || ENV.fetch("GITHUB_REPO_NAME", "Arions")
    @owner = config[:owner] || ENV.fetch("GITHUB_REPO_OWNER", "KostoLabs")
    @branch = config[:branch] || ENV.fetch("GITHUB_REPO_BRANCH", "Continous-Integration")
  end
  ...
  def fetch_latest_upgrade_candidates
    Rails.cache.fetch("latest_github_upgrade_candidates", expires_in: 30.minutes) do
      ...
      begin
        ...
        latest_version = latest_release ? Semver.from_release_tag(latest_release.tag_name) : Semver.new(Arion.version)
